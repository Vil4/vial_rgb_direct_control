...
def hid_send(dev, msg, retries=1):
    ...
    while retries > 0:
        ...
        try:
            ...
            data = bytes(dev.read(MSG_LEN, timeout=500))
            if not data:
                continue
        except OSError:
            continue
...
def is_rawhid(desc):
    ...
    dev = hid.Device(desc["vendor_id"],desc["product_id"],path=desc['path'])
...
def vialrgb_send_leds(dev, leds):
    ...
    while sent < num_leds:
        ...
        if not any(x.need_update == True for x in leds_to_send):
...
def main():
    ...
    while not done:
        ...
        for led in leds:
            ...
            if led == active_led:
                color = (255, 255, 255)
            pygame.draw.circle(screen, color=color, center=(led.render_x+8, led.render_y+8), radius=20)
